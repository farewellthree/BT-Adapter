...
class VideoChatGPTLlamaModel(LlamaModel):
    ...
    def initialize_vision_modules(self, mm_vision_tower=False, pretrain_mm_mlp_adapter=None, tune_mm_mlp_adapter=False, pretrain_mm_vision_tower=None):
        if mm_vision_tower:
            ...
            self.vision_tower = build_btadapter_from_CLIP(CLIP_tower, weight=pretrain_mm_vision_tower, training=True).half()
            del CLIP_tower
    ...
    def forward(
        ...
        #video_spatio_temporal_features = video_spatio_temporal_features[:, :256]
        vision_tower = getattr(self, 'vision_tower', None)
        
        if vision_tower is not None:
            video_spatio_temporal_features = vision_tower(video_spatio_temporal_features).hidden_states[-1]
        ...
        if (input_ids.shape[1] != 1 or self.training) and video_spatio_temporal_features is not None:
            ...
            #video_features = checkpoint.checkpoint(self.mm_projector,video_spatio_temporal_features)
            ...
            #dummy_video_features = checkpoint.checkpoint(self.mm_projector,dummy_video_features)
...
class BTAdapterLlamaForCausalLM(LlamaForCausalLM):
    config_class = VideoChatGPTConfig
...
AutoModelForCausalLM.register(VideoChatGPTConfig, BTAdapterLlamaForCausalLM)
