...
from bt_adapter.train.llava_trainer import BTAdapterLLavaTrainer
from bt_adapter import video_conversation as conversation_lib
from bt_adapter.model import *
...
class ModelArguments:
    ...
    mm_vision_tower: bool = field(default=False)
    freeze_CLIP: bool = field(default=True)
    pretrain_mm_vision_tower: Optional[str] = field(default=None)
    convert_type: bool = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    clip_lr: float = field(default=0)
    llama_lr: float = field(default=0)
...
def train():
    ...
    model = BTAdapterLlamaForCausalLM.from_pretrained(
    ...
    model_vision_dict = model.get_model().initialize_vision_modules(
        ...
        pretrain_mm_vision_tower=model_args.pretrain_mm_vision_tower,
    ...
    #if model_args.convert_type:
    #    if training_args.fp16:
    #        model.to(dtype=torch.float16)
    #    elif training_args.bf16:
    #        model.to(dtype=torch.bfloat16)
    if model_args.mm_vision_tower:
        training_args.mm_vision_tower = model_args.mm_vision_tower
        vision_tower = getattr(model.get_model(), 'vision_tower')
        vision_tower.requires_grad_(False)
        if not model_args.convert_type:
            vision_tower.to(dtype=torch.float32)
        if model_args.freeze_CLIP:
            for name,p in vision_tower.named_parameters():
                if 'btadapter' in name and '3' not in name:
                    p.requires_grad = True
        else:
            for name,p in vision_tower.named_parameters():
                p.requires_grad = True
    ...
    trainer = BTAdapterLLavaTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
