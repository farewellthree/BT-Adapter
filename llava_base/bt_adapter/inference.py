from bt_adapter.video_conversation import conv_templates, SeparatorStyle
from bt_adapter.model.utils import KeywordsStoppingCriteria
import torch
...
def video_conversation_infer(video_frames, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len):
    """
    ...
    with torch.no_grad():
        ...
        frame_features = image_forward_outs.hidden_states[-2] # Use second to last layer as in LLaVA

    if isinstance(frame_features,tuple):
        x1, x2 = frame_features
        x1, x2 = x1[:,1:], x2[:,1:]
        x2 = rearrange(x2, 'b (t l) d -> (b t) l d', t=x1.size(0))
        video_spatio_temporal_features = x1.mean(0) + x2.mean(0)
        video_spatio_temporal_features = video_spatio_temporal_features.half()
    else:
        #if len(frame_features.size())==3:
        #    video_spatio_temporal_features = frame_features.squeeze().half()
        #else:
        #video_spatio_temporal_features = frame_features[:, 1:]
        video_spatio_temporal_features = frame_features.mean(dim=0)

        #video_spatio_temporal_features = get_spatio_temporal_features_torch(video_spatio_temporal_features)
